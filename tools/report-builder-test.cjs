...
async function ratingFlagTests() {
  ...
  const asked = await loadShared({ total: 1500 }).NXTK.prepareRatingPrompt();
  assert.equal(asked.milestone, 1500, 'the prompt is prepared for the milestone that came due');
  assert.match(asked.copy, /1500 files downloaded/, 'and its copy names the count');
  assert.match(asked.listing.reviewUrl, /^https:\/\//, 'with the listing it links to');
  assert.equal(await loadShared({ total: 3 }).NXTK.prepareRatingPrompt(), null,
    'and there is nothing to prepare below the first milestone');
}

// Every writer stores settings through the same bounds, so a value typed into a field can never
// reach storage out of range, and every rate-limit wait is computed the same way.
function settingLimitTests() {
  const { NXTK } = loadShared();
  const normalize = NXTK.normalizeSetting;
  assert.equal(normalize('NDC_pauseBetweenDownload', -5), 0, 'a negative pause is stored as none');
  assert.equal(normalize('NDC_pauseBetweenDownload', 99999), 600, 'and a huge one is capped');
  assert.equal(normalize('RequestTimeout', 1000), 5000, 'a timeout too short to reach Nexus is raised');
  assert.equal(normalize('CloseTabDelay', 1500.6), 1501, 'millisecond settings are whole numbers');
  assert.equal(normalize('NDC_downloadSpeed', 2.5), 2.5, 'the speed keeps its decimals');
  assert.equal(normalize('NDC_downloadMethod', 7), 1, 'the method is one of the two that exist');
  assert.equal(normalize('DownloadFolder', 'x'.repeat(300)).length, 100, 'the folder name is capped');
  assert.equal(normalize('AutoStartDownload', 'yes'), undefined, 'a value of the wrong type is refused');
  assert.equal(normalize('RequestTimeout', Number.NaN), undefined, 'and so is a number that is not one');
  assert.equal(normalize('NotASetting', 1), undefined, 'and a key that does not exist');

  assert.equal(NXTK.parseRetryAfterSeconds('120'), 120, 'Retry-After in seconds');
  assert.equal(NXTK.parseRetryAfterSeconds(''), null, 'no header, no answer');
  assert.equal(NXTK.parseRetryAfterSeconds('soon'), null, 'an unreadable header is ignored');
  assert.ok(NXTK.parseRetryAfterSeconds(new Date(Date.now() + 60000).toUTCString()) >= 58,
    'an HTTP date counts down to itself');
  assert.deepStrictEqual([1, 2, 3, 20].map((strike) => NXTK.rateLimitBackoffSeconds(strike)), [30, 60, 120, 600],
...
function ratingSurfaceTests() {
  ...
  for (const [label, source] of [['deck', ui], ['popup', popup]]) {
    assert.match(source, /prepareRatingPrompt/, `${label}: the shared helper decides whether to ask`);
    ...
    assert.match(source, /prompt\.starText/, `${label}: GitHub is offered beside the review`);
    assert.match(source, /prompt\.copy/, `${label}: the copy names the count`);
  ...
  const promptStart = shared.indexOf('async function prepareRatingPrompt(');
  const prompt = shared.slice(promptStart, shared.indexOf('\n  }\n', promptStart));
  assert.match(prompt, /dueRatingMilestone/, 'the milestone decides whether to ask');
  assert.match(prompt, /ratingStarCta/, 'GitHub is offered beside the review');
  assert.match(prompt, /ratingPromptCount/, 'the copy names the count');
