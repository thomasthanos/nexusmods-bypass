...
function importFileFilterTests() {
  // The list is shared with the worker and the download namer, so it comes from shared.js.
  const shared = fs.readFileSync('src/shared.js', 'utf8');
  const NXTK = {
    ARCHIVE_FILE_EXTENSIONS: new Function(`return ${extractConstExpression(shared, 'ARCHIVE_FILE_EXTENSIONS')};`)()
  };
  const api = new Function('Array', 'RegExp', 'NXTK', `
...
function storeListingBehaviour() {
  ...
  // Both surfaces ask through one helper, so the listing and its wording are pinned there once.
  const prompt = extractFunction(shared, 'prepareRatingPrompt');
  ...
  for (const [label, source] of [['deck', ui], ['popup', popup]]) {
    ...
    assert.match(source, /prompt\.listing\.reviewUrl/, `${label}: links to the resolved listing`);
  }
  assert.match(ui, /stars\.setAttribute\('aria-hidden', 'true'\)/, 'deck: the stars are decoration, not a control');
  assert.match(fs.readFileSync('src/popup/popup.html', 'utf8'), /id="ratingStars" aria-hidden="true"/,
    'popup: the stars are decoration, not a control');
...
// Every dialog is built by openModal. Enter on a focused Cancel used to confirm "Restore defaults",
...
function dialogRegressionTests() {
  ...
  assert.match(extractFunction(ui, 'openModal'), /getTopModalBackdrop\(\) !== backdrop/,
    'only the dialog on top answers the keyboard');
  assert.match(extractFunction(ui, 'showUpdateModal'), /if \(run !== compareRun/,
    'a stale revision comparison cannot render');
  assert.match(extractFunction(ui, 'importWabbajackModlist'), /deckRunIsActive/,
    'a modlist import never replaces a deck whose run is still going');
  assert.match(extractFunction(ui, 'disposeControlDeck'), /deckOwners\.get\(deck\)\?\.dispose\?\.\(\)/,
    'removing a deck ends the run it was showing');
}

// A worker verdict that needs the reader to act (a Cloudflare check, a suspended account) used to end a
// browser run as a bare "Download ended with an error", with nothing saying what to do about it.
function blockingQueueOutcomeTests() {
  const ndc = fs.readFileSync('src/content/ndc.js', 'utf8');
  const at = ndc.indexOf('const finish = (outcome');
  assert.notEqual(at, -1, 'the queue watcher still settles through finish');
  const finish = ndc.slice(at, ndc.indexOf('\n        };', at));
  assert.match(finish, /Errors\.isBlocking\(\{ code \}\)/, 'the verdict is checked for being blocking');
  assert.match(finish, /'blocked'/, 'such a run ends as blocked');
  assert.match(finish, /showError/, 'with the dialog that explains it');
  assert.match(finish, /queueErrorText\(reason\)/, 'and the log names the reason in words');
  assert.doesNotMatch(ndc, /TERMINAL_STATUS/, 'the status poll shares the outcomes the events use');
  assert.match(ndc, /message\.startId !== this\.backgroundStartId/,
    'before START answers, only events from this start are adopted');
}

// Firefox serves extension files from moz-extension://, where the old chrome-extension:// check
// meant no uncaught extension error was ever recorded.
function extensionErrorFilterTests() {
  const main = fs.readFileSync('src/content/main.js', 'utf8');
  const build = (chrome) => new Function('chrome',
    `${extractFunction(main, 'isExtensionScript')}; return isExtensionScript;`)(chrome);

  const firefox = build({ runtime: { id: 'x', getURL: () => 'moz-extension://abc/' } });
  assert.equal(firefox('moz-extension://abc/content/ui.js'), true, 'a Firefox extension file is recognised');
  assert.equal(firefox('https://www.nexusmods.com/assets/app.js'), false, 'a page script is not');

  const chromium = build({ runtime: { id: 'x', getURL: () => 'chrome-extension://abc/' } });
  assert.equal(chromium('chrome-extension://abc/content/nnw.js'), true);
  assert.equal(chromium('chrome-extension://other/content.js'), false, 'another extension is not ours');

  const reloaded = build({ runtime: {} });
  assert.equal(reloaded('moz-extension://abc/content/ui.js'), true,
